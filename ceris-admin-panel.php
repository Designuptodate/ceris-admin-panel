<?php
    /*
    Plugin Name: Ceris Admin Panel
    Plugin URI: https://themesific.com
    Description: Ceris theme admin panel
    Author: BKNinja
    Version: 1.6
...
function bk_theme_welcome() {
	...
	if (current_user_can('manage_options')) {  
	   //add_menu_page( $page_title, $menu_title, $capability, $menu_slug, $function, $icon_url, $position );
        add_menu_page(esc_html__('Ceris Demo Import', 'bkninja'), esc_html__('Ceris Demo Import', 'bkninja'), 'edit_theme_options', 'bk-theme-welcome', 'bk_welcome_template', 'dashicons-admin-site', 4 );
    }
}
...
function bk_theme_demos() {
	// Check that the user is allowed to update options  
	if (current_user_can('manage_options')) { 
        if(get_option('required_plugins')) {
